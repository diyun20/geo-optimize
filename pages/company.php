...
geoInitTables();
$pageTitle = "企业信息 - GEO优化";

try {
    dbExecute("CREATE TABLE IF NOT EXISTS `company_info` (
        `id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        `user_id` INT UNSIGNED NOT NULL,
        `company_name` VARCHAR(200) DEFAULT '',
        `company_abbr` VARCHAR(100) DEFAULT '',
        `region` VARCHAR(200) DEFAULT '',
        `short_video_account` VARCHAR(200) DEFAULT '',
        ...
        `trust_endorsements` TEXT DEFAULT NULL,
        `user_pain_points` TEXT DEFAULT NULL,
        `customer_cases` TEXT DEFAULT NULL,
...
$info = dbFetchOne("SELECT * FROM company_info WHERE user_id = ?", [$user["id"]]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($info) {
        ...
        dbExecute($sql, [$data["company_name"], $data["company_abbr"], $data["region"], $data["short_video_account"], $data["industry"], $data["products_services"], $data["product_highlights"], $data["brand_story"], $data["trust_endorsements"], $data["user_pain_points"], $data["customer_cases"], $user["id"]]);
    } else {
        ...
        dbExecute($sql, [$user["id"], $data["company_name"], $data["company_abbr"], $data["region"], $data["short_video_account"], $data["industry"], $data["products_services"], $data["product_highlights"], $data["brand_story"], $data["trust_endorsements"], $data["user_pain_points"], $data["customer_cases"], date("Y-m-d H:i:s")]);
    }
    dbExecute("UPDATE company_info SET short_video_account = ? WHERE user_id = ?", [trim($_POST["short_video_account"] ?? ""), $user["id"]]);
    setFlash("success", "企业信息已保存");
    redirect("index.php?route=company");
}
?>
...
<form method="post" action="index.php?route=company">
    <div class="knowledge-grid">
        ...
        <div class="knowledge-card">
            ...
            <h3>产品名称</h3>
            <div class="form-group">
                <label>产品名称（多个用换行分隔）</label>
                <textarea name="products_services" rows="4" placeholder="每行一个产品名称，例如：&#10;线上美高学分课程&#10;美国高中留学申请&#10;AP大学先修课程"><?= h($info["products_services"] ?? "") ?></textarea>
            </div>
            <div class="form-group">
                <label>产品提点</label>
